...
- 全自动点歌姬，自动变化的界面，以及播放时间预计
- 没有版权的歌曲自动更换播放源
- 单个粉丝设置特别关心、强提醒、专属昵称等
- 小窗闲聊、迷你视频，上班摸鱼挂后台听直播必备
- 根据礼物价值，使用不同语气程度答谢，发送多条来表达惊叹
- 查看每名观众进来次数、礼物总计、弹幕记录、判断机器人等
- 按等级、粉丝牌等多个条件，并筛选关键词自动禁言
- 大量可编程运算，超高自由度的弹幕机器人
- 支持小游戏，比如扣1禁言、赠送小心心解禁言等
- 更多神奇的隐藏功能（不敢公开）
...
| {key}            | 获取配置文件中的值       | key为setValue(key)中的键值                    |
...
另外，仅支持在开头的`[]`判断中进行四则运算，**不支持在弹幕内容中直接计算**！
...
赠送一个喵娘则提前5首歌播放。可将`5`改为`999`表示无限大，~~或者用 `%gift_gold% / 1000` 表示每1000金瓜子礼物可提前一首歌~~（暂不支持函数中计算）
