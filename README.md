...
这一类事件专门用于在某些操作之前进行过滤。

当结果的弹幕包含 `>reject()` 时，将不会执行正在过滤的操作。

若有多行，同其他代码一样，随机获取一行；其他弹幕正常发送

| 过滤名              | 说明（空弹幕时）   |
| ------------------- | ------------------ |
| FILTER_MUSIC_ORDER  | 阻止点歌           |
| FILTER_DANMAKU_MSG  | 阻止显示在弹幕姬上 |
| FILTER_DANMAKU_COME | 阻止显示在弹幕姬上 |
| FILTER_DANMAKU_GIFT | 阻止显示在弹幕姬上 |



##### 示例：点歌过滤

禁止未戴本房间粉丝勋章或勋章等级小于5的用户的点歌

事件：`FILTER_MUSIC_ORDER`

```
["%anchor_room_id%" != "%room_id%"; %medal_level% < 5]>reject()
```

两个条件：`未戴本房间勋章`、或`勋章等级<5`，当成立时，执行 `>reject()` ，即**阻止操作**。
