...
| getDanmuHistory(uid, count[, format, type, options])        | 获取弹幕历史     | 获取历史弹幕。uid 为 0 时不指定账号；count 指定数量；format 为每行格式，默认：`{time(MM-dd hh:mm)} {uname}:{text}`；type可为：line（英文分号分隔的单行纯文本）、text（使用`%n%`分隔的多行纯文本，用来作为参数）、list（直接使用换行符分隔）、gpt（GPT格式输入）、json（JSON数组）；options 为更多选项，例如包含 `word_count < 1000` 时指定字数和少于1000 |
