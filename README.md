...
| aiChat(text, code)                                 | AI聊天           | 调用AI回复文字，若有结果则执行code代码，使用 `\%text\%` 来获取回复结果 |
...
| 事件命令           | 说明                                                         |
| ------------------ | ------------------------------------------------------------ |
| LIVE_RECORD_SAVED  | 录播结束，`%path%` 获取视频文件的绝对路径，`%file_name%` 获取文件名（不包含后缀） |
| LIVE_RECORD_FORMAT | 开启“保存为mp4”后，转换格式成功时触发此事件                  |
