...
- [说明书](#说明书)
  - [使用技巧](#使用技巧)
    - [实时弹幕](#实时弹幕)
    - [设置账号Cookie](#设置账号cookie)
    - [欢迎弹幕](#欢迎弹幕)
        - [示例：只欢迎戴自己勋章](#示例只欢迎戴自己勋章)
        - [示例：指定用户欢迎语](#示例指定用户欢迎语)
    - [送礼答谢](#送礼答谢)
        - [示例：指定礼物专属答谢](#示例指定礼物专属答谢)
        - [示例：不答谢禁言用户](#示例不答谢禁言用户)
    - [关注答谢](#关注答谢)
        - [示例：特别关注](#示例特别关注)
    - [定时任务](#定时任务)
        - [示例：今日是否有大航海](#示例今日是否有大航海)
        - [示例：定时联网](#示例定时联网)
    - [自动回复](#自动回复)
        - [示例：关键词自动禁言](#示例关键词自动禁言)
        - [示例：房管远程弹幕禁言](#示例房管远程弹幕禁言)
        - [示例：不要欢迎我](#示例不要欢迎我)
        - [示例：设置自己专属昵称](#示例设置自己专属昵称)
        - [示例：修改指定用户昵称](#示例修改指定用户昵称)
    - [浏览器插件](#浏览器插件)
    - [远程控制](#远程控制)
    - [弹幕样式](#弹幕样式)
    - [多开与备份](#多开与备份)
  ...
  - [可编程设计](#可编程设计)
    ...
    - [命令操作](#命令操作)
      - [批量修改配置](#批量修改配置)
      - [显示值列表](#显示值列表)
        - [示例：显示所有用户积分](#示例显示所有用户积分)
        - [示例：显示所有打卡记录](#示例显示所有打卡记录)
        - [示例：显示用户总金瓜子](#示例显示用户总金瓜子)
      - [解析JSON：网络数据回调](#解析json网络数据回调)
        - [示例：获取主播信息](#示例获取主播信息)
      - [简单命令示例](#简单命令示例)
        - [示例：自动打卡](#示例自动打卡)
        - [示例：禁言小游戏](#示例禁言小游戏)
        ...
        - [示例：送礼优先点歌](#示例送礼优先点歌)
        - [示例：弹幕切歌](#示例弹幕切歌)
        - [示例：远程开关AI弹幕回复](#示例远程开关ai弹幕回复)
        - [示例：远程添加违禁词](#示例远程添加违禁词)
    - [事件动作](#事件动作)
      - [主程序事件](#主程序事件)
        - [示例：远程禁言回复](#示例远程禁言回复)
      - [时间事件](#时间事件)
        ...
        - [示例：高级打卡](#示例高级打卡)
      - [点歌姬事件](#点歌姬事件)
        - [示例：点歌提示未带勋章](#示例点歌提示未带勋章)
        - [示例：点歌后立即切歌](#示例点歌后立即切歌)
      - [弹幕事件](#弹幕事件)
        - [示例：复读机](#示例复读机)
        - [示例：QQ群推送开播消息](#示例qq群推送开播消息)
        - [示例：下播自动关机](#示例下播自动关机)
        - [示例：感谢分享直播间](#示例感谢分享直播间)
        - [示例：上船声音提示](#示例上船声音提示)
        - [示例：上船自动设置房管](#示例上船自动设置房管)
        - [示例：成为第x位船长](#示例成为第x位船长)
        - [示例：恭喜勋章升级](#示例恭喜勋章升级)
        - [示例：天选时刻中奖播报](#示例天选时刻中奖播报)
      - [大乱斗事件](#大乱斗事件)
        - [示例：大乱斗结束前提醒](#示例大乱斗结束前提醒)
        - [示例：大乱斗蹭积分卡](#示例大乱斗蹭积分卡)
        - [示例：大乱斗最佳助攻](#示例大乱斗最佳助攻)
        - [示例：大乱斗尊严票](#示例大乱斗尊严票)
        - [示例：自动开启大乱斗](#示例自动开启大乱斗)
        - [示例：主播互相串门](#示例主播互相串门)
        - [示例：大乱斗匹配信息](#示例大乱斗匹配信息)
        - [示例：大乱斗连胜](#示例大乱斗连胜)
        - [示例：对面在线舰长播报](#示例对面在线舰长播报)
      - [PK对面事件](#pk对面事件)
      - [过滤器事件](#过滤器事件)
        - [示例：点歌过滤](#示例点歌过滤)
        - [示例：只显示舰长进入](#示例只显示舰长进入)
        - [示例：过滤免费礼物](#示例过滤免费礼物)
        - [示例：自定义过滤器](#示例自定义过滤器)
        - [示例：自定义词库过滤](#示例自定义词库过滤)
        - [示例：自定义正则过滤](#示例自定义正则过滤)
      - [警告事件](#警告事件)
      - [服务端事件](#服务端事件)
        - [示例：远程计数](#示例远程计数)
  - [<div id='web_dev'/>](#div-idweb_dev)
  - [Web开发接口](#web开发接口)
    - [浏览器访问](#浏览器访问)
    - [Socket通讯](#socket通讯)
    - [主程序发送消息](#主程序发送消息)
...
（V4.0.6新增）当条件表达式包含字符串或者正则表达式时，使用双方括号 `[[条件]]` 来替换单方括号。
...
机器人发送弹幕之后，至少间隔他人的多少弹幕才会发送。

如定时消息：

```
(cd11:30,wait12:3)某定时发送的弹幕
```

自动发送一条弹幕后，不会在30秒内发送，并且第一条和第二条弹幕之间至少有3条弹幕。

如果不满足条件，则不会发送出去。

**等待通道必须和冷却通道一起使用，否则无效。**
